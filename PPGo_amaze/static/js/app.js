...
function splitData(rawData) {
    ...
    var values = [];
    var volumes = [];
	var tmpList = rawData.split("],[")
    for (var i = 0; i < tmpList.length; i++) {
		var tmpValue = tmpList[i].replace(/"/g, "");
		var valueList = tmpValue.replace(/\[/g, "").replace(/\]/g, "").split(",");
        categoryData.push(valueList[0]);
    	var tmp = [];
		tmp.push(parseFloat(valueList[1]));
		tmp.push(parseFloat(valueList[2]));
		tmp.push(parseFloat(valueList[3]));
		tmp.push(parseFloat(valueList[4]));
		tmp.push(parseFloat(valueList[5]));
        values.push(tmp);
        //values.push(rawData[i]);
        volumes.push([i, parseFloat(valueList[5]), valueList[1] > valueList[2] ? 1 : -1]);
	}
...
//    }
    ...
    //var rawData = document.getElementById('Slider1').innerHTML;
	var rawData = $("#Slider1").text();
...
//                }
    ...
    echartsC.dispatchAction({
        ...
        areas: [
            {
                brushType: 'lineX',
                coordRange: ['2017-12-01', '2017-12-29'],
